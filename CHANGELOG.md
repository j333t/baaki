...
- **Not its own button, in the end.** The first version gave Since a third kind-picker button, its own quick-chip row (yesterday, a week ago, a month ago...), and hid the calendar entirely, on the theory that a calendar offering only future days couldn't help with a past pick. All three were the same idea taught three times over: this is a date already gone. Simpler fix - stop disabling past days in the calendar at all. Pick one for an ordinary deadline and it becomes a since automatically; pick a future one and it's a deadline, same as always. One mechanism, not two, and a screen's worth of UI deleted rather than added.
- A bare day already past lands at the *start* of that day, not the end - "since Monday" means Monday as it began. Same rule as before, now triggered by the date being past rather than by which button was pressed. Something good never flips this way - arriving late is still arriving, not a start to run from.
- The past-date warning ("that is in the past, use this if you mean it") no longer fires for a deadline - a past pick isn't a mistake any more, it's the whole mechanism. Something good keeps the warning; arriving late is still worth a second look.

### A fourth kind of goal: Recurring
- **Comes back on its own - weekly, monthly, or yearly.** "Next 1-on-1," "rent due," "board meeting" are real recurring commitments, and deserved the same one-number pressure as any other goal without needing to be re-created every cycle. Three hand-rolled patterns, deliberately not RRULE - that spec has far more surface than this tool would ever use, and it breaks the plain-English voice every other date in this file already has.
- **No new fields to store the pattern itself.** The weekday, the day-of-month, the month - all already sit in the target date. A recurring goal only needs one extra byte on the wire: which of those three readings applies.
- **Done marks that cycle, not the series.** Pressing Done behaves exactly like an ordinary deadline - early, on time, or late, same colours, same feedback - and only once the *actual due moment* arrives does the goal quietly reset to the next one. Marking it done three days early doesn't jump the queue three days early; the "done, early" state stays on screen until the date it was actually about arrives.
- **A miss stays a miss.** An unmarked recurring goal sits overdue exactly like any other missed deadline - it never silently rolls forward on its own, and even a very late Done only ever advances one cycle, never catches up to "today" in a jump.
- **No new rendering path.** Once a completed cycle rolls over, a recurring goal is byte-for-byte an ordinary deadline to every other part of the file - colour, the overdue mirror, the chip row, all of it. The only new logic is the one check, run once a second alongside the render loop, that decides whether to roll.
- **Colour is keyed on the name alone**, not name-plus-target like everything else - a recurring goal's target moves every cycle on its own, and losing its colour every time would undo the whole point of a goal having one.
- **The marker is two characters (`#w`/`#m`/`#y`), not one.** A bare trailing letter risks colliding with a real timezone name ending the same way - Europe/Moscow, Pacific/Guam, Europe/Jersey. `#` never appears in an encoded zone name, so the pair stays unambiguous regardless.

### A fifth kind of goal: Window
- **Two dates, one link.** "Exam 10am-noon," "boarding opens-gate closes" are one thing with two moments, not two separate goals pretending to be related. A window counts down to its start like something good, then the instant the start arrives it switches to counting down to its end like a deadline - whoever opens the link on the day gets the right view automatically, with nothing to coordinate.
- **The switch is instant and needs no new rendering.** Both render paths - something good, deadline - already existed; the only new logic is a phase check that decides which target and style apply this second. One number on screen at a time, never two competing for it.
- **No Done until the switch has happened.** Something good never has a Done button, and up to the start a window is rendering as exactly that. The moment it flips to counting down to the end, Done follows the ordinary deadline rule, same as it always has.
- **The extra date is stored only on this kind.** Deadline, something good, since, and recurring stay exactly as they were - single date, zero extra bytes - because needing a second date for one kind never justified spending it on the kinds that don't.
- **No marker character - the link's own shape is the tell.** A second `~` before the target (`name~start~target`) says "this is a window" on its own, and a window is never combined with `*`/`^`/`#w`/`#m`/`#y` - it is its own kind, not a modifier on one of those.
- **One set of date-picking controls, used twice, not two sets at once.** Every other kind needed exactly one date, so the Goals form has only ever had one calendar/quick-picks/typed-field group. Picking a start, then being asked for an end with the same controls, costs one extra tap - not a second, permanently-idle copy of the whole picker sitting in a dialog that already rejected a two-column layout twice for being wasteful.
- **Editing a window jumps straight to its end**, with a small back link to revisit the start - the end is the date actually being adjusted almost every time.

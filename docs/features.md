...
| ★ Five kinds - deadline, event (`*`), since (`^`), recurring (`#w`/`#m`/`#y`), window (`start~target`) - but four buttons | Not everything you count toward is a threat, not everything worth watching has an end, not everything happens once, and not everything is a single moment. A deadline heats up and runs over; something good glows, arrives, and stops; since counts up from a start, forever; recurring comes back on its own; a window has a start and an end, both of which matter. Since isn't a separate button: pick a day already past for a deadline and it becomes one automatically. The past was always the tell - a button asking you to say so a second time was the redundant part. |
...
| The calendar no longer disables the past | It used to offer only today forward, on the theory that the past had nothing useful to select. Since proved that wrong - a past day is a real, common answer, just for a different kind of goal. Past days are dimmed, not disabled; Prev has nowhere it needs to stop either, now that any day is a valid pick. |
| `N unit ago` and `yesterday` were already in the typed vocabulary | Mirrors "in N unit" going forward - the same short words, the other direction. Nothing new here; the calendar catching up to what typing could already do is what changed. |
| A bare day already past starts at midnight, not 11:59pm | A deadline's bare day means "by the end of it." Once that day is already gone, the opposite reading is the sane one: "since it began." Same parser, same default, now triggered by the date being past rather than by which button was pressed. |
| Typing or picking a past deadline gets no warning | Baaki used to say "that is in the past, use this if you mean it" for any backward-looking deadline. Once past-for-a-deadline always means since, the warning was describing a mistake that no longer exists. Something good keeps its warning - arriving late is still a slip worth flagging. |

## Recurring

| Feature | Why it exists |
|---|---|
| ★ A goal that comes back on its own - weekly, monthly, or yearly | "Next 1-on-1," "rent due," "board meeting" benefit from the same ambient-glance pressure as a one-off, but shouldn't need re-creating every cycle. Three hand-rolled patterns, not RRULE - that spec has far more surface (BYSETPOS, EXDATE, COUNT...) than this tool would ever use, and typing `FREQ=WEEKLY;BYDAY=MO` breaks the plain-English voice every other date field already has. |
| No pattern fields of its own - the anchor date already carries them | The weekday, the day-of-month, the month-and-day are all already sitting in `target`. A repeating goal only needs one extra byte: which of the three readings applies. |
| Done marks that one instance, not the series | "Done" on a recurring goal means this cycle is finished, not that the goal is over - delete is what actually ends it. Marking it early shows early/on-time/late exactly like any other deadline; nothing new to learn. |
| It only advances once its own due moment arrives, not the moment you press Done | Marking rent paid on the 28th shouldn't instantly show next month's countdown three days early - the "done, 3 days early" state is the interesting thing to look at until the 1st actually comes around. Waiting for the real due instant means the celebration always gets a chance to show before the board moves on. |
| A missed cycle sits overdue, exactly like a missed deadline - it does not silently roll forward | Recurring never forgives a miss by hiding it behind next month's goal. It only ever advances one cycle per Done, however late that Done is pressed - never catches up to "today" in one jump. |
| Same render path as an ordinary deadline, no special case | Once due-and-done rolls the target and clears done, a recurring goal is indistinguishable from a fresh deadline to every other part of the file - colour, the overdue mirror, the chips, all of it. The only new code is the one check that decides whether to roll. |
...
| The extra date is stored only on this kind | Deadline, something good, since, and recurring all stay single-date, exactly as before - zero extra bytes for goals that never needed a second date. |
| The link's own tell, no marker character | A second `~` before the target - `name~start~target` - says "this is a window" on its own. Never combined with `*`/`^`/`#w`/`#m`/`#y`: a window is its own kind, not a modifier on one of those. |
| Added to the Goals form as a two-step pick, reusing the same calendar/quick/typed widgets twice, not two of them at once | Every other kind needed exactly one date, so the form has only ever had one set of date-picking controls. Building a second, permanent one just for this kind would double the dialog's height for everyone, including the four kinds that don't need it - the "one column, no dead space" rule that killed a two-column layout twice already argues against a second calendar sitting there unused too. Picking a start, then being asked for an end with the same controls, costs one extra tap instead of double the dialog. |
| Editing an existing window jumps straight to its end, with a back link to revisit the start | The end is the date most likely being adjusted; making every edit start by re-confirming the start it almost never changes would be the redundant tap in the other direction. |
...
| An unnamed goal gets a phrase, not a number | "1", "2", "3" told you nothing and looked like an error. A placeholder's one job is to be obviously a placeholder — nobody should mistake one for a label somebody chose, least of all whoever you sent the link to. |
| One register, and nothing grim | Everyday things and vague stretches of time, so the joke is that the same board counts down to the washing up and to one of these days. "heat death" was drafted and cut: a board saying that over somebody's surgery date is not funny. |
| The pick avoids what is already on the board | Random is only useful if it cannot collide. Two goals called "bin day" is worse than one called "2". |
| A repeated name takes a number, wherever it came from | Applies to the phrase list and to a name merged in from somebody else's link alike — "Keep both" on two goals called "Exam" was leaving exactly the confusion the question was asked to avoid. |
| The front door is bottle green, off the ramp | The empty board is the only screen not counting anything real, so letting the deadline ramp paint it meant it wore whatever colour the days left in the year happened to land on — teal in September, amber by December. The ramps run navy through teal to amber to red, so green is the one direction nothing else occupies: a front door should not look like a deadline in progress. |
...
| The card is flat, not a gradient | On screen the gradient is slow and alive and you look at it for weeks. In a 1200x630 crop glanced at for half a second in a chat list it just reads as an uneven wash. Flat is also kinder to the JPEG it has to be. |
| The card is the same colour as the screen it opens | A preview should feel like a door into the thing, not an advert for it. `og-card.mjs` asks the empty board what it is painted rather than being told. |
...
| Every share hands over the site root, not `/baaki.html` | `index.html` forwards the fragment untouched, so the root is the same board in a shorter, better-looking link — and it is the page a crawler finds the preview card on, which the file name is not. A `file://` copy and the desktop wrapper have no root, so they keep the path they were opened from. |
...
| Four custom events: goal added, marked done, shared, QR drawn | An open isn't use. These are the four moments that are. Fires a bare event name only — never a goal's name, date, or count. |

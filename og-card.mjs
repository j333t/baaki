/* Draws og.png - the picture WhatsApp, Slack and Discord show instead
   ...
   page with nothing on it, asks what its own front door is painted,
   and uses that. Nudge the colour and re-run, and the card follows.

   Flat, not a gradient. On screen the gradient is slow and alive and
   you are looking at it for weeks; in a 1200x630 crop seen for half a
   second in a chat list it just reads as an uneven wash. Flat is also
   kinder to JPEG, which is what this has to be.
...
/* Ask the real page what its empty board is painted, rather than
   guessing. The card and the screen somebody lands on should be the
   same colour - that is most of what makes a preview feel like a door
   into the thing rather than an advert for it. */
const probe = await ctx.newPage();
await probe.goto(FILE);
await probe.evaluate(() => { localStorage.removeItem('baaki.hash'); localStorage.removeItem('baaki.prev'); });
...
.num{font-size:300px;font-weight:600;line-height:.92;letter-spacing:-.02em;
  display:flex;align-items:flex-start}
.num sup{font-size:.26em;font-weight:600;opacity:.5;margin-top:.12em;margin-left:.04em}
...
console.log(`${OUT}  ${kb.toFixed(1)} KB  flat ${ramp.g2}`);
